...
const SORT_LABEL: Record<string, string> = {
  price_asc: "Menor preço", price_desc: "Maior preço",
  year_desc: "Mais novos",  year_asc: "Mais antigos",
}
    ...
    <section className="grid gap-4">
      ...
      <div className="grid gap-3 bg-white rounded-2xl p-4 border">
        <div className="grid grid-cols-2 md:grid-cols-8 gap-3">
          <input
            placeholder="Marca" value={filters.brand ?? ""}
            onChange={(e) => setFilter({ brand: e.target.value || undefined })}
            className="rounded border px-3 py-2"
          />
          <input
            placeholder="Modelo" value={filters.model ?? ""}
            onChange={(e) => setFilter({ model: e.target.value || undefined })}
            className="rounded border px-3 py-2"
          />
          <input
            placeholder="Ano" type="number" value={filters.year ?? ""}
            onChange={(e) => setFilter({ year: e.target.value || undefined })}
            className="rounded border px-3 py-2"
          />
          <select
            value={filters.status ?? ""} onChange={(e) => setFilter({ status: e.target.value || undefined })}
            className="rounded border px-3 py-2"
          >
          ...
          <input
            placeholder="Preço mín." type="number" value={filters.price_min ?? ""}
            onChange={(e) => setFilter({ price_min: e.target.value || undefined })}
            className="rounded border px-3 py-2"
          />
          <input
            placeholder="Preço máx." type="number" value={filters.price_max ?? ""}
            onChange={(e) => setFilter({ price_max: e.target.value || undefined })}
            className="rounded border px-3 py-2"
          />
          <select
            value={filters.sort ?? ""} onChange={(e) => setFilter({ sort: e.target.value || undefined })}
            className="rounded border px-3 py-2"
        ...
        <div className="flex flex-wrap items-center gap-2">
          {(["brand","model","year","status","price_min","price_max"] as const).map((k) => {
            ...
            if (!val) return null
            const label =
              k === "price_min" ? `Preço ≥ ${currency.format(Number(val))}` :
              k === "price_max" ? `Preço ≤ ${currency.format(Number(val))}` :
              k === "status" ? (val === "available" ? "Disponível" : val === "reserved" ? "Reservado" : "Vendido") :
              `${k}=${val}`
            return (
              <button key={k} onClick={() => setFilter({ [k]: undefined } as any)} className="px-2 py-1 text-sm rounded-full border bg-gray-50 hover:bg-gray-100">
                {label} ✕
              </button>
            )
          })}
          {filters.sort && (
            <button onClick={() => setFilter({ sort: undefined })} className="px-2 py-1 text-sm rounded-full border bg-gray-50 hover:bg-gray-100">
              {SORT_LABEL[filters.sort] ?? `ordenar=${filters.sort}`} ✕
            </button>
          )}
          <button onClick={clearAll} className="text-sm underline underline-offset-2">Limpar filtros</button>
        </div>
      </div>
            ...
            <tbody>
              {d.data.map((v: Vehicle) => (
                <tr key={v.id} className="border-t">
                  <td className="px-4 py-3"><ImageThumb images={v.images} /></td>
                  <td className="px-4 py-3">{v.id}</td>
                  <td className="px-4 py-3">{v.brand}</td>
                  <td className="px-4 py-3">{v.model}</td>
                  <td className="px-4 py-3">{v.year}</td>
                  <td className="px-4 py-3">{v.status === "available" ? "Disponível" : v.status === "reserved" ? "Reservado" : "Vendido"}</td>
                  <td className="px-4 py-3">{currency.format(Number(v.price))}</td>
                  <td className="px-4 py-3 text-right">
                    <div className="flex gap-2 justify-end">
                      <Link to={`/vehicles/${v.id}`} className="px-2 py-1 rounded border hover:bg-gray-50">Editar</Link>
                    </div>
                  </td>
                </tr>
              ))}
